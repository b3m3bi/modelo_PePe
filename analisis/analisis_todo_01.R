...
data <-
    ...
    mutate(captura_viaje =
               if_else(num_viajes_mes == 0,
                       0, sum_capturas_mes / num_viajes_mes),
           horas_en_mar_viaje =
               if_else(num_viajes_mes == 0,
                       0, sum_horas_en_mar_mes / num_viajes_mes),
           distancia_recorrida_viaje =
               if_else(num_viajes_mes == 0,
                       0, sum_distancias_recorridas_mes / num_viajes_mes),
           gasto_gas_viaje =
               if_else(num_viajes_mes == 0,
                       0, sum_gasto_gas_mes / num_viajes_mes),
           ganancia_viaje =
               if_else(num_viajes_mes == 0,
                       0, sum_ganancias_mes / num_viajes_mes))
...
cols_salida <- names(data[, 10:ncol(data)])
...
data_mean_series <- data %>%
    group_by(numero_embarcaciones,
             ...
             meses_transcurridos) %>%
    summarize(across(cols_salida, mean))
