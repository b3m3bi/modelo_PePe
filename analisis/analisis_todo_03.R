...
data <- read_csv("data/modelo_PePe analisis_todo_03-table.csv", skip = 6)
...
ggplot(data_mean_series,
       aes(x = meses_transcurridos,
           y = sum_distancias_recorridas_mes,
...
data_mean_series <- data %>%
    filter(numero_embarcaciones == 100,
           porcentaje_anp %in% c(0,20),
           numero_plataformas %in% c(0,10,20)) %>%
    group_by(
        meses_transcurridos,
        numero_plataformas,
        porcentaje_anp) %>%
