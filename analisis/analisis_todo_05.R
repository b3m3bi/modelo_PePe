...
data <- read_csv("data/modelo_PePe analisis_todo_05-table.csv", skip = 6)
...
cols_salida <- names(data[, 11:ncol(data)])
...
data_mean_series <- data %>%
    group_by(
        ...
        m,
        capturabilidad) %>%
    summarize_at(cols_salida,
                 list(mean = mean,
                      max = max,
                      min = min))

data_mean_series %>%
    filter(porcentaje_anp == 0,
           r == 0.7) %>%
    ggplot(aes(x = meses_transcurridos,
               y = sum_distancias_recorridas_mes_mean / num_viajes_mes_mean,
               fill = as.factor(numero_plataformas)
               )) +
    geom_line(aes(color = as.factor(numero_plataformas))) +
    geom_ribbon(aes(ymin = sum_distancias_recorridas_mes_min / num_viajes_mes_mean,
                    ymax = sum_distancias_recorridas_mes_max / num_viajes_mes_mean),
                alpha = 0.3 ) +
    facet_grid(cols = vars(capturabilidad),
               rows = vars(m)) +
    labs(x = "Mes",
         y = "Distancia recorrida",
         color = "Número plataformas",
         fill = "Número plataformas")


data_mean_series %>%
    filter(porcentaje_anp == 20
           ) %>%
    ggplot(aes(x = meses_transcurridos,
               y = sum_distancias_recorridas_mes_mean / num_viajes_mes_mean,
               fill = as.factor(numero_plataformas)
    ...
    geom_line(aes(color = as.factor(numero_plataformas),
                  linetype = as.factor(porcentaje_anp))) +
    ## geom_ribbon(aes(ymin = sum_distancias_recorridas_mes_min / num_viajes_mes_mean,
    ##                 ymax = sum_distancias_recorridas_mes_max / num_viajes_mes_mean),
    ##             alpha = 0.3 ) +
    facet_grid(cols = vars(capturabilidad),
...
data_mean_series %>%
    ...
    ggplot(aes(x = meses_transcurridos,
               y = mean_salario_mensual_of_embarcaciones_mean,
               fill = as.factor(numero_plataformas)
               )) +
    geom_line(aes(color = as.factor(numero_plataformas),
                  linetype = as.factor(porcentaje_anp))) +
    geom_ribbon(aes(ymin =  mean_salario_mensual_of_embarcaciones_min,ymax = mean_salario_mensual_of_embarcaciones_max),alpha = 0.3 ) +
    facet_grid(cols = vars(capturabilidad),
               rows = vars(m)) +
    labs(x = "Mes",
         y = "Ingreso mensual",
...
data_mean_series %>%
    ...
    ggplot(aes(x = meses_transcurridos,
               y =mean_salario_ultimo_ano_mean,
               fill = as.factor(numero_plataformas)
               )) +
    geom_line(aes(color = as.factor(numero_plataformas),
                  linetype = as.factor(porcentaje_anp))) +
    geom_ribbon(aes(ymin = mean_salario_ultimo_ano_min,ymax = mean_salario_ultimo_ano_max),alpha = 0.3 ) +
    facet_grid(cols = vars(capturabilidad),
               rows = vars(m)) +
    labs(x = "Mes",
         y = "Ingreso mensual",
...
data_mean_series %>%
    ...
    ##        ) %>%
    ggplot(aes(x = meses_transcurridos,
               y = gasto_gas_acumulado_mean / num_viajes_acumulado_mean,,
               fill = as.factor(numero_plataformas)
               )) +
    geom_line(aes(color = as.factor(numero_plataformas),
                  linetype = as.factor(porcentaje_anp))) +
...
data_mean_series %>%
    ...
    geom_line(aes(color = as.factor(numero_plataformas))) +
    geom_ribbon(aes(ymin = mean_salario_mensual_of_embarcaciones_min,ymax = mean_salario_mensual_of_embarcaciones_max), alpha = 0.3 ) +
    facet_grid(cols = vars(porcentaje_anp),
               ) +
    labs(x = "Mes",
...
data_mean_series %>%
    ...
    geom_line(aes(color = as.factor(numero_plataformas))) +
    geom_ribbon(aes(ymin = gasto_gas_acumulado_min / num_viajes_acumulado_mean,ymax = gasto_gas_acumulado_max / num_viajes_acumulado_mean), alpha = 0.3 ) +
    facet_grid(cols = vars(porcentaje_anp),
               ) +
    labs(x = "Mes",
         y = "Gasto gas",
